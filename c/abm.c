...
simulation_t new_simulation(int identity, int n_agents, int n_infections)
{
  simulation_t s;
  s.identity = identity;
  int n = n_agents * 3 / 2;
...
/*
 * Count the number of agents by a given state.
 */
int count_agents_by_state(const simulation_t *simulation, state_t state)
{
  int total = 0;
  for (int i = 0; i < simulation->size; i++)
    if (simulation->agents[i].state == state) ++total;
  return total;
}

/*
 * Count the number of agents not in a given state.
 */
int count_agents_not_by_state(const simulation_t *simulation, state_t state)
{
  int total = count_agents_by_state(simulation, state);
  return simulation->size - total;
}
...
void grow(simulation_t *simulation, double growth_per_day) {
  int num_agents = count_agents_not_by_state(simulation, DEAD);
...
void infect(simulation_t *simulation, int events)
{
  for (int i = 0; i < events; i++) {
	...
	simulation->agents[ind2].state == INFECTED) {
      simulation->agents[ind1].state = INFECTED;
    } else if (simulation->agents[ind1].state == INFECTED &&
 ...
 * agents.
 */
void die(simulation_t *simulation, double death_rate_susceptible, double death_rate_infected)
{
  for (agent_t *agent = simulation->agents;  agent < simulation->agents + simulation->size; agent++) {
...
void report(const simulation_t *simulation, int iteration)
{
  ...
  int num_infections = count_agents_by_state(simulation, INFECTED);
  int num_deaths = count_agents_by_state(simulation, DEAD);
  // We want to send one string to puts to prevent data races on stdout
  char output[250];
  snprintf(output, 250,
	   "Simulation: %d. Iteration: %d. Susceptible: %d. Infections: %d. Deaths: %d\n",
	   simulation->identity, iteration, num_susceptible, num_infections, num_deaths);
  puts(output);
}
 ...
 */
void simulate(simulation_t *simulation, int iterations, double growth_per_day, int events,
	      double death_rate_susceptible, double death_rate_infected)
{
  for (int i = 0; i < iterations; i++) {
    ...
    if (i % 100 == 0) report(simulation, i);
  }
}
...
int main(int argc, char **argv) {
  ...
  pool = g_thread_pool_new((GFunc)run_one_simulation,
			   (gpointer) &parameters,
